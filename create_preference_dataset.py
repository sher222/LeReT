...
class CreatePreferenceDataset:
    ...
    def generate_fewshot(self, threads=20, canidates=4, file_prefix="", out_path=""):
        ...
        basicmh_bs = tp.compile(
            SingleHop(), trainset=trainset[:50], valset=trainset[50:100]
        )
    ...
    def __init__(
        ...
        if self.tgi_server:
            self.tgiserver = TGIServer(
                device_ids=tgi_device_ids,
                model_archive=model_archive,
                model_name=model_name,
                gpu_memory=tgi_gpu_memory,
            )
            self.tgiserver.prepare_model()
            self.tgiserver.start(verbose=tgi_verbose)
            self.lm = customdspy.tgi_chat.TGIChat(
                model=self.model_name, port=3002, url="http://localhost"
            )
        else:
            self.lm = dspy.Together(
                model=utils.get_together_model_name(self.model_name),
                max_tokens=1024,
                stop=["<|eot_id|>"],
        ...
        self.retriever = (
            ColBERTv2Local()
            if local_retriever
            else dspy.ColBERTv2(url="http://20.102.90.50:2017/wiki17_abstracts")
        )
    ...
    def make_nth_dataset(
        ...
        for i, filepath in enumerate(ensemble_filepaths):
            ...
            _, out, _, _ = evaluate.evaluate(
                data_path,
                "train",
                output_prefix=f"{file_template}_{filepath_end}",
                num_iterations=self.num_iterations,
                iteration=self.iteration,
        ...
        for index, arr in enumerate(data):
            for j in arr:
                dataset[j["example_id"]].append({**j, "model": index})
        ...
        for i, arr in enumerate(data):
            for j in arr:
                ...
                dataset[p].append({**j, "model": i, "prompt": p})
        ...
        for k, v in dataset.items():
            ...
            sorted_v = list(
                filter(
                    lambda x: "Context: [1]" not in x[hop_key]["search_query"], sorted_v
                )
            )
            if len(sorted_v) == 0:
                continue
            preference_data[prompt]["responses"] = [
                sorted_v[i][hop_key]["search_query"] for i in range(len(sorted_v))
            ]
            ...
            preference_data[prompt]["sft_target"] = sorted_v[-1][hop_key][
                "search_query"
            ]
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Create a preference dataset using prompt driven diverse sampling algorithm."
    )
    parser.add_argument(
        "--model_name", type=str, required=True, help="Name of the model to use."
    )
    parser.add_argument(
        "--model_archive", type=str, default=None, help="Path to the model archive."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=str,
        default="hotpotqa",
        choices=["hotpotqa", "hover"],
        help="Name of the dataset.",
    )
    parser.add_argument(
        "--tgi_gpu_memory", type=int, default=1, help="GPU memory for TGI server."
    )
    parser.add_argument(
        "--together_api_key", type=str, default=None, help="API key for Together."
    )
    parser.add_argument(
        "--tgi_verbose", type=bool, default=False, help="Verbose mode for TGI server."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--hosted_retriever",
        action="store_true",
        help="Use hosted retriever instead of local.",
    )
    parser.add_argument(
        "--num_threads", type=int, default=64, help="Number of threads to use."
    )
    ...
    preference_files = [
        out_file + f"hop{i}_preference.json" for i in range(1, num_hops + 1)
    ]
