...
class TGIServer:
    ...
    def get_slurm_device_ids(self):
        ...
        command = "scontrol show job $SLURM_JOB_ID -d | grep IDX:"
    ...
    def __init__(
        ...
        self.device_ids = (
            self.get_slurm_device_ids() if device_ids == "unknown" else device_ids
        )
    ...
    def prepare_model(self, overwrite=False):
        ...
        model_path = (
            self.model_archive
            if self.model_archive[0] == "/"
            else f"{self.cache_dir}/{self.model_archive}"
        )
        ...
        policy = transformers.AutoModelForCausalLM.from_pretrained(
            self.model_name, cache_dir=self.cache_dir, low_cpu_mem_usage=True
        )
        tokenizer = transformers.AutoTokenizer.from_pretrained(
            self.model_name, cache_dir=self.cache_dir
        )
        state_dict = torch.load(
            os.path.join(model_path, "policy.pt"), map_location="cpu"
        )
    ...
    def start(self, verbose=True):
        ...
        print(
            "starting tgi container with path",
            path,
            "mount_path",
            mount_path,
            "port",
        ...
        self.container = client.containers.run(
            ...
            device_requests=[
                docker.types.DeviceRequest(
                    device_ids=[self.device_ids], capabilities=[["gpu"]]
                )
            ],
            shm_size="1g",
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Start a TGI server with specified parameters."
    )
    parser.add_argument(
        "--model_name",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to the model archive, should be to folder containing optimizer.pt, policy.pt, scheduler.pt",
    )
    parser.add_argument(
        "--device_ids",
        type=str,
        default="unknown",
        help="Comma-separated list of device IDs or 'unknown' to get from SLURM.",
    )
    parser.add_argument(
        "--port", type=int, default=3002, help="Port to run the TGI server on."
    )
    parser.add_argument(
        "--gpu_memory", type=float, default=1.0, help="Fraction of GPU memory to use."
    )
    parser.add_argument(
        "--cache_dir", type=str, default="./cache", help="Directory to cache models."
    )
    parser.add_argument(
        "--hf_token",
        type=str,
    ...
    parser.add_argument(
        "--max_total_tokens", type=int, default=8192, help="Maximum total tokens."
    )
    parser.add_argument(
        "--max_input_tokens", type=int, default=8000, help="Maximum input tokens."
    )
