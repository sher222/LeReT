...
class SingleHop(dspy.Module):
    ...
    def __init__(self, passages_per_hop=3):
        ...
        self.generate_query = [dspy.ChainOfThought("context, question -> search_query") for _ in range(1)]
    ...
    def forward(self, question, context=[]):
        ...
        for hop in range(1):
            ...
            search_query = self.generate_query[hop](context=text_context, question=question).search_query
            resp = self.retrieve(search_query)
            passages = [{"long_text": i["long_text"], "score": i["score"]} for i in resp.passages]
...
def save_model(prog, path):
    ...
    serialized = state_str.replace("Example", "dspy.Example").replace("(input_keys=None)", "")
