...
def get_leret_dataset(path, tokenizer, model_name):
    ...
    for k, v in data.items():
        if "gemma" in model_name:
            chat = [{
                "role": "user",
                "content": "You are a helpful assistant. You must continue the user text directly without *any* additional interjections." + k
            }]
        else:
            chat = [
        ...
        key = tokenizer.apply_chat_template(chat, tokenize=False, add_generation_prompt=True)
        
        processed_data[key] = v
    return processed_data

def get_dataset(name, tokenizer, model_name):
    """Load the given dataset by name. Supported by default are 'shp', 'hh', and 'se'."""
    data = get_leret_dataset(name, tokenizer, model_name)
...
def get_batch_iterator(names: List[str],
    ...
    with TemporarilySeededRandom(seed):
        ...
        for name in names:
            ...
            for prompt, data in get_dataset(name, tokenizer, model_name).items():
...
if __name__ == '__main__':
    ...
    iterator = get_batch_iterator(
        ["/iris/u/sherylh/RLaIR/data/createpreferencedataset/test_codebase_hotpot/combined.json"],
        tokenizer,
        split='train',
        batch_size=8,
        max_length=2048,
