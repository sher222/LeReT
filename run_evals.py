...
class RunEvals:
    ...
    def __init__(
        ...
        for models_counter, model_archive in enumerate(models):
            ...
            print(
                "on model", model_name, "count", models_counter, "out of", len(models)
            )
            tgi_args = {
            ...
            if model_archive == "base":
                ...
                output_name = (
                    ...
                    else model_archive.replace("/", "_")
                )
                if tgi_server:
                    tgi_args["model_archive"] = model_archive
                else:
                    raise Exception(
                        "TGI server must be enabled to evaluate custom models."
                    )
            if tgi_server:
                ...
                evaluate.update_lm(
                    dspy.Together(
                        model=utils.get_together_model_name(model_name),
                        max_tokens=1024,
                        stop=["<|eot_id|>"],
            ...
            for split in splits.split(","):
                ...
                # first hop
                file, _, _, _ = evaluate.evaluate(
                    dataset_name,
                    split,
                    output_prefix=f"{output_name}/hop1",
                    hop_number=1,
                ...
                for hop in range(2, num_hops + 1):
                    file, _, _, _ = evaluate.evaluate(
                        file,
                        split,
                        output_prefix=f"{output_name}/hop{hop}",
                        hop_number=hop,
                        num_threads=num_threads,
                    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--models", type=str, required=True, help="List of models to evaluate."
    )
    parser.add_argument(
        "--splits", type=str, default="dev", help="Dataset splits to evaluate on."
    )
    parser.add_argument(
        "--tgi_device_ids",
        type=str,
        default="unknown",
        help="Device IDs for TGI server.",
    )
    parser.add_argument(
        "--model_name",
        type=str,
    ...
    parser.add_argument(
        "--gpu_memory", type=float, default=1.0, help="GPU memory allocation."
    )
    parser.add_argument(
        "--tgi_verbose",
        action="store_true",
        help="Enable verbose logging for TGI server.",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="hotpotqa",
        help="Name of the dataset, hotpotqa or hover.",
    )
    parser.add_argument("--tgi_server", action="store_true", help="Use TGI server.")
    parser.add_argument(
        "--num_threads", type=int, default=128, help="Number of threads for evaluation."
    )
    ...
    parser.add_argument(
        "--cache_dir", type=str, default="./cache", help="Directory for cache."
    )
