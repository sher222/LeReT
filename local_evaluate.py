...
class LocalEvaluate:
    ...
    def evaluate(
        ...
        if output_prefix is None:
            ...
            output_filepath = (
                f"{self.cache_dir}/{output_prefix}_{dataset_output_name}_{split}.json"
            )
        ...
        if dataset_filepath in ["hotpotqa", "hover"]:
            ...
            dataset = (
                HotpotRetrievalDataset(**dataset_args)
                if dataset_filepath == "hotpotqa"
                else HoverRetrievalDataset(**dataset_args)
            )
            sets = {
                "train": dataset.train,
                "dev": dataset.dev,
            ...
            with open(dataset_filepath, "r") as f:
                data = json.load(f)
            dataset = [
                dspy.Example({**i, "context": i["full_retrieved"]}) for i in data
            ]
        ...
        for j in tqdm.tqdm(
            range(already_done, len(selected_dataset), save_every),
            desc=f"grouped by {save_every}",
        ):
            print(
                f"for outfile {output_filepath}: on {j} out of {len(selected_dataset)}"
            )
            evaluate_hotpot = Evaluate(
                devset=selected_dataset[j : j + save_every],
                metric=EM_Metric,
            ...
            for k in sub_examples:
                ...
                hop_dict = {
                    f"hop{hop_number}": {
                        ...
                        "score": {"em": k[2], "ap": AP_Metric(k[0], k[1])},
